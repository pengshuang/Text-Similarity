...
'''
 implement of the paper: "ABCNN: Attention-Based Convolutional Neural Network for Modeling Sentence Pairs"
'''
class ABCNN3(nn.Module):
    ...
    def forward(self, *input):
        ...
        mask1, mask2 = s1.eq(constant.PAD_INDEX), s2.eq(constant.PAD_INDEX)
        res = [[], []]
        s1, s2 = self.embeds(s1), self.embeds(s2)
        batch_size, seq_len, dim = s1.shape
        res[0].append(F.max_pool1d(s1.transpose(1, 2), kernel_size=s1.size(1)).squeeze(-1))
        res[1].append(F.max_pool1d(s2.transpose(1, 2), kernel_size=s2.size(1)).squeeze(-1))
...
class Wide_conv(nn.Module):
    def __init__(self, seq_len, embeds_size):
        ...
        self.conv = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=[3,3], padding=[1, 1], stride=1)
        self.fc = nn.Sequential(
            # Swish(),
        )
    ...
    def forward(self, sent1, sent2, mask1, mask2):
        ...
        A = match_score(sent1, sent2, mask1, mask2) # A: batch_size * seq_len * seq_len
        
        attn_feature_map1 = A.matmul(self.W) # attn_feature_map1: batch_size * seq_len * dim
        attn_feature_map2 = A.transpose(1, 2).matmul(self.W)
        x1 = torch.cat([sent1.unsqueeze(1), attn_feature_map1.unsqueeze(1)], 1) # x1: batch_size * 2 *seq_len * dim
        x2 = torch.cat([sent2.unsqueeze(1), attn_feature_map2.unsqueeze(1)], 1)
        o1, o2 = self.conv(x1).squeeze(1), self.conv(x2).squeeze(1)
        return self.fc(o1), self.fc(o2)
